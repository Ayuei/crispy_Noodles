...
def get_parameters():
    ...
    for layer_index, neural_layer_vals in enumerate(NN_DIMENSION):
        ...
        if layer_index < len(NN_DIMENSION):
            print('Initializing layer ', 'W' + str(layer_index), ' with size ', NN_DIMENSION[layer_index], 'x', NN_DIMENSION[layer_index-1])
            params['W' + str(layer_index)] = np.random.randn(NN_DIMENSION[layer_index], NN_DIMENSION[layer_index-1]) / 1000
...
def get_cost(outputValue, Y, numrecords):
    ...
    cost = - (1 / numrecords) * np.sum(np.multiply(Y, np.log(outputValue)) + np.multiply(1 - Y, np.log(1 - outputValue)))
...
def testNN(imageFilePath, actualLabel, trainedWeights):
    ...
    print ("Provided label is:" , bool(actualLabel), ". NN predicts:", result, ".  Predicted chance of TRUE =", '{:.1%}'.format(classifier_percentage.item(0)))
